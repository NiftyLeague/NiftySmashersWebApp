...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (displayName && EntityToken)
      await ChangeDisplayName(displayName, EntityToken);
    // Update Profile Contact Email
    if (email && SessionTicket)
      await AddOrUpdateContactEmail(email, SessionTicket);
    // Update Profile Avatar
    if (avatar_url && SessionTicket)
      await UpdateAvatarUrl(avatar_url, SessionTicket);
